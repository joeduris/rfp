...
int main(int argc, char *argv[]) {
   ...
   double intensity_scale_factor = 1., xlamds, dgrid, A, B, D, Lu, Lc, dl, df, cutradius = 0;
   ...
   if(argc >= narglim++) {
      ...
      dgrid = strtod(argv[iarg++],0);
      Lu = strtod(argv[iarg++],0);
      Lc = strtod(argv[iarg++],0);
      dl = strtod(argv[iarg++],0);
      df = strtod(argv[iarg++],0);
   }
   if(argc >= narglim++) {
   ...
   // do a single file
   return radfieldprop(input_dfl_filename, output_dfl_filename, xlamds, dgrid, dgrid, A, B, D, intensity_scale_factor, ncar, nslip, verboseQ, cutradius);
